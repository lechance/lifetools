...
`birthday-countdown` 工具仅支持**阳历生日**（不支持农历）：主页面不占表单空间，右下角**浮动 ＋ 按钮**点击弹出**底部弹窗**（`bd__popup` + `bd__fab`，`showForm` 控制）填写人员（称呼/关系/生日，生日用 `<picker mode="date">` 存 `birthYear` + 月/日）。按年循环计算倒计时，日历视图按阳历月日匹配标记。卡片展示**年龄**（周岁，`getAge` 已过生日算实岁、未到减一）与**星座**（阳历月日计算，函数内联在页面内），下次生日行附带「满 X 岁」。**日历提醒**：仅微信小程序，每张人员卡片底部「📅 添加日历提醒」调用 `wx.addPhoneRepeatCalendar` 写入系统日历（按年重复，生日当天 09:00 开始，提醒提前量 = `remindDays` 设置，即顶部「提前提醒」输入框（可自定义 0-365 天整数，0 为当天）；iOS 对非预设提前量如 3/14/30 天可能拒绝，已做 `alarmOffset:0` 回退）。成功后人员记录写 `calendarAdded: true` 持久化。微信**无删除/更新日历事件 API**：取消提醒需用户在系统日历手动删除，改名/删除人员不会同步日历（卡片重加时弹确认说明）。H5 端点击仅 toast 提示不支持。
